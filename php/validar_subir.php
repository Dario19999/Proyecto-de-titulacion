<?php
    ...
    while($row = mysqli_fetch_array($res_groseria)){
        ...
        //     if(strpos(${"ingr_" . $i}, $row['groseria']) !== false){
        //         $bandera = 0;
    ...
    if($bandera == 1){
        try{
            ...
            $insert_receta = $conexion->prepare("INSERT INTO receta (id_nacionalidad, id_usuario, id_categorias, nombre_receta) VALUES (?, ?, ?, ?)");
            ...
            for($j = 1; $j<=$cant_ingr; $j++){
                $insert_ingr = $conexion->prepare("INSERT INTO datos_receta (id_receta, nombre_ingrediente, porciones, cantidad, medida) VALUES (?, ?, ?, ?, ?)");
